...
export async function generateStaticParams() {
  return CLUSTERS.map((c) => ({ slug: c.slug.en }))
}

export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const cluster = getClusterBySlug(params.slug, 'en')
  if (!cluster) return {}
  return {
    title: cluster.title.en,
    description: cluster.description.en,
    alternates: {
      canonical: `${BASE}/blog/topics/${cluster.slug.en}`,
      languages: {
    ...
    openGraph: {
      title: cluster.title.en,
      description: cluster.description.en,
      url: `${BASE}/blog/topics/${cluster.slug.en}`,
      type: 'website',
    },
  }
}

export default async function ClusterHubPageEN({ params }: { params: { slug: string } }) {
  const cluster = getClusterBySlug(params.slug, 'en')
  if (!cluster) notFound()

  const articles = cluster.articleSlugs.en
    .map((slug) => getPost(slug, 'en'))
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      ...
      <Link href="/blog" className="text-sm text-[var(--muted)] hover:text-white transition-colors mb-6 inline-block">
        ← Blog
      </Link>
      ...
      <section className="mb-12">
        <h2 className="text-xs font-black uppercase tracking-widest text-[var(--muted)] mb-5">
          <span aria-hidden="true">📖</span> Articles in this hub
        </h2>
      ...
      <section className="mb-12">
        <h2 className="text-xs font-black uppercase tracking-widest text-[var(--muted)] mb-5">
          <span aria-hidden="true">🗳</span> Vote on live dilemmas
        </h2>
          ...
          {cluster.scenarioCategories.map((cat) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-bold text-sm text-white">{meta?.label ?? cat}</span>
              </Link>
            )
          })}
        </div>
      ...
      <div className="mt-12 pt-8 border-t border-[var(--border)] text-center">
        <p className="text-xs text-[var(--muted)] mb-3">Want everything in one place?</p>
        <Link
          href="/blog"
